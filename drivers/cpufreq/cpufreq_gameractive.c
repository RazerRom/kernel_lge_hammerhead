/*
 *  drivers/cpufreq/cpufreq_gameractive.c
 *
 ...
 *            (C)  2015 gamerman123x <19superguy98@gmail.com>
 *
...
static unsigned int skip_gameractive = 0;
static u64 gameractive_freq_boosted_time;
...
static int gameractive_sampling_rate_boosted;
static u64 gameractive_sampling_rate_boosted_time;
unsigned int gameractive_current_sampling_rate;
static void do_dbs_timer(struct work_struct *work);
static int cpufreq_governor_dbs(struct cpufreq_policy *policy,
unsigned int event);

#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_GAMERACTIVE
static
#endif
struct cpufreq_governor cpufreq_gov_gameractive = {
       .name                   = "gameractive",
				...
				const char *buf, size_t count)
{
	...
	gameractive_freq_boosted_time = ktime_to_us(ktime_get());

	if (gameractive_sampling_rate_boosted) {
		gameractive_sampling_rate_boosted = 0;
		dbs_tuners_ins.sampling_rate = gameractive_current_sampling_rate;
	}
	return count;
}
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	if (dbs_tuners_ins.boosted && policy->cpu == 0) {
		if (ktime_to_us(ktime_get()) - gameractive_freq_boosted_time >=
	...
	if (gameractive_sampling_rate_boosted && policy->cpu == 0) {
		if (ktime_to_us(ktime_get()) - gameractive_sampling_rate_boosted_time >=
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	return cpufreq_register_governor(&cpufreq_gov_gameractive);
}
...
static void __exit cpufreq_gov_dbs_exit(void)
{
	...
	cpufreq_unregister_governor(&cpufreq_gov_gameractive);
	for_each_possible_cpu(i) {
		struct cpu_dbs_info_s *this_dbs_info =
...
MODULE_DESCRIPTION("'cpufreq_gameractive' - multiphase dynamic-hybrid cpufreq governor");
MODULE_LICENSE("GPL");

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_GAMERACTIVE
